...
function parseRateLines(lines: string[]): { code: string; against: string; buy: string; sell: string }[] {
  ...
  // Matches: "USD / ZWG 24.4829 25.2401 25.9973"  or  "ZWG/ ZAR 0.6320 0.6515 0.6711"
  // Group 1 = code, 2 = against, 3 = bid/buy, 4 = mid (ignored), 5 = offer/sell
  const rowRe = /^([A-Z]{2,4})\s*\/\s*([A-Z]{2,4})\s+([\d.,]+)\s+([\d.,]+)\s+([\d.,]+)/;

  for (const line of lines) {
    const m = line.trim().match(rowRe);
    if (!m) continue;
    const code    = m[1].toUpperCase();
    const against = m[2].toUpperCase();
    const buy     = m[3].replace(/,/g, '');
    // m[4] = mid rate — intentionally ignored
    const sell    = m[5].replace(/,/g, '');

    // Only keep pairs where "against" is one of the three display currencies
    if (!['USD', 'ZWG', 'ZAR'].includes(against)) continue;
